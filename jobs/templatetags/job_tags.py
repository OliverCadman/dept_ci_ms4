...
def append_page_no_to_url_params(context, **kwargs):
    ...
    and link is:
    <a href={% url "find_a_dep" % }
            {% append_page_no_to_url_params page=3 %}>
    </a>

    the returned URL will append "&page=2" to the end of the current URL,
    instead of replacing/forgetting about the already
    existing queries, resulting in:
