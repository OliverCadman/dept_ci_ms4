(function (window) {
  /*
  Script to render the
  SoundManager Music Player in the Profile Page.

  Code acquired from blog, after consulting SoundManager docs:
  https://rickluna.com/wp/2016/02/how-to-use-soundmanager2-with-bar-ui/
...
*/
  ...
  Player = function (playerNode) {
    ...
    extras = {
      loadFailedCharacter:
        '<span title="Failed to load/play." class="load-error">✖</span>',
    };
    ...
    function setTitle(item) {
      ...
      if (
        dom.playlistTarget.getElementsByTagName("li")[0].scrollWidth >
        dom.playlistTarget.offsetWidth
      ) {
        // this item can use <marquee>, in fact.
        dom.playlistTarget.innerHTML =
          '<ul class="sm2-playlist-bd"><li><marquee>' +
          item.innerHTML +
          "</marquee></li></ul>";
      }
    }
        ...
        whileplaying: function () {
          ...
          left =
            Math.min(
              ...
              Math.max(
                0,
                progressMaxLeft * (this.position / this.durationEstimate)
              )
            ) + "%";
          width =
            Math.min(
        ...
        onerror: function () {
          ...
          if (item) {
            ...
            if (extras.loadFailedCharacter) {
              dom.playlistTarget.innerHTML =
                dom.playlistTarget.innerHTML.replace(
                  "<li>",
                  "<li>" + extras.loadFailedCharacter + " "
                );
              if (
                playlistController.data.playlist &&
                playlistController.data.playlist[
          ...
          if (navigator.userAgent.match(/mobile/i)) {
            ...
            playlistController.data.timer = window.setTimeout(
              actions.next,
              2000
            );
          }
        ...
        onfinish: function () {
          ...
          if (
            ...
            (playlistController.data.selectedIndex !== lastIndex ||
              (playlistController.data.playlist.length === 1 &&
    ...
    function getActionData(target) {
      ...
      actionData.volume.backgroundSize = parseInt(
        utils.style.get(target, "background-size"),
        10
      );

      // IE gives pixels even if background-size specified as % in CSS. Boourns.
      if (window.navigator.userAgent.match(/msie|trident/i)) {
        actionData.volume.backgroundSize =
          (actionData.volume.backgroundSize / actionData.volume.width) * 100;
      }
    }
    ...
    actions = {
      ...
      shuffle: function (e) {
        ...
        var target = e ? e.target || e.srcElement
          : utils.dom.get(dom.o, ".shuffle");

        if (target && !utils.css.has(target, css.disabled)) {
          utils.css.toggle(target.parentNode, css.active);
          playlistController.data.shuffleMode =
            !playlistController.data.shuffleMode;
        }
      },
      ...
      menu: function (ignoreToggle) {
        ...
        // playlist
        dom.playlistContainer.style.height =
          (isOpen ? dom.playlistContainer.scrollHeight : 0) + "px";
      },
      ...
      adjustVolume: function (e) {
        ...
        if (!e || clientX === undefined) {
          ...
          if (arguments.length && window.console && window.console.warn) {
            console.warn(
              "Bar UI: call setVolume(" +
                e +
                ") instead of adjustVolume(" +
                e +
                ")."
            );
          }
        ...
        value = Math.max(
          0,
          Math.min(1, (clientX - actionData.volume.x) / actionData.volume.width)
        );

        target.style.clip =
          "rect(0px, " +
          actionData.volume.width * value +
          "px, " +
          actionData.volume.height +
          "px, " +
          actionData.volume.width * (backgroundMargin / 100) +
          "px)";
        ...
        volume =
          Math.max(
            ...
            Math.min(
              1,
              (clientX - actionData.volume.x - pixelMargin) /
                (actionData.volume.width - pixelMargin * 2)
            )
          ) * 100;
      ...
      setVolume: function (volume) {
        ...
        if (dom.volume) {
          ...
          to =
            from +
            (actionData.volume.width - backgroundOffset * 2) * (volume / 100);
  ...
  utils = {
    ...
    css: (function () {
      function hasClass(o, cStr) {
        return o.className !== undefined ? new RegExp("(^|\\s)" + cStr + "(\\s|$)").test(o.className)
          : false;
      }
    ...
    events: (function () {
      ...
      remove =
        window.removeEventListener !== undefined ? function (o, evtName, evtHandler) {
      ...
      getClientX = function (e) {
        ...
        return (
          e && (e.clientX || (e.touches && e.touches[0] && e.touches[0].pageX))
        );
      };
